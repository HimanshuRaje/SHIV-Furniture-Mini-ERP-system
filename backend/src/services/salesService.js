...
const confirmSalesOrderAutomated = async (id, userId) => {
  const so = await prisma.salesOrder.findUnique({
    where: { id },
    include: {
      lines: {
  ...
  await prisma.$transaction(async (tx) => {
    for (const line of so.lines) {
      ...
      const freeToUseQty = product.onHandQty - product.reservedQty;
      const reserveQty = Math.min(Math.max(freeToUseQty, 0), line.qty);
      const shortfallQty = line.qty - reserveQty;
    ...
    await tx.auditLog.create({
      data: {
        ...
        afterData: {
          status: 'CONFIRMED',
          autoProcurementNeeds: procurementNeeds,
        },
      },
    });
  });
  ...
  for (const need of procurementNeeds) {
    try {
      ...
      procurementNotifications.push({
        ...
        procurementStrategy: need.procurementStrategy,
        orderedQty: need.orderedQty,
        reservedQty: need.reservedQty,
        shortageQty: need.qty,
      });
    } catch (error) {
      procurementNotifications.push({
        type: 'ERROR',
...
module.exports = {
  ...
  updateSalesOrder,
  deleteSalesOrder,
  confirmSalesOrder: confirmSalesOrderAutomated,
  deliverSalesOrder,
  cancelSalesOrder,
};
