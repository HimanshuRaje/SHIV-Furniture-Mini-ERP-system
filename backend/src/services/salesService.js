...
const generateInvoiceNo = async () => {
  ...
  // Find the sales order with the highest invoiceNo for the current year
  ...
  // Double check that it does not exist (in case of deletes or concurrency)
  let invoiceNo = `INV-${year}-${String(nextSeq).padStart(6, '0')}`;
  let exists = await prisma.salesOrder.findUnique({ where: { invoiceNo } });
  while (exists) {
    nextSeq++;
    invoiceNo = `INV-${year}-${String(nextSeq).padStart(6, '0')}`;
    exists = await prisma.salesOrder.findUnique({ where: { invoiceNo } });
  }
...
const createSalesOrder = async ({
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      const isUniqueConstraint = error.code === 'P2002' && error.meta && error.meta.target && error.meta.target.includes('invoice_no');
      const wasAutoGenerated = !invoiceNo;

      if (isUniqueConstraint && wasAutoGenerated && attempts < maxAttempts) {
        console.warn(`Unique constraint failed on auto-generated invoiceNo "${resolvedInvoiceNo}". Retrying... Attempt ${attempts}/${maxAttempts}`);
        continue;
      }
      throw error;
    }
